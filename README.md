...
ORM (Object-Relational Mapping) – технологія програмування, яка пов'язує бази даних із концепціями об'єктно-орієнтованих мов програмування, створюючи «віртуальну об'єктну базу даних». Існують як пропрієтарні, і вільні реалізації цієї технології.

Загалом технологія ORM дозволяє проектувати роботу з даними в термінах класів, а не таблиць даних. Вона дозволяє перетворювати класи на дані, придатні для зберігання базі даних, причому схему перетворення визначає сам розробник. Крім того, ORM надає простий API-інтерфейс для CRUD-операцій над даними. Завдяки технології ORM немає потреби писати SQL-код для взаємодії з локальною базою даних.

Серед переваг ORM виділяють:
...
- Можливість оперувати елементами мови програмування, тобто. класами, об'єктами, атрибутами, методами, а чи не елементами реляційної моделі даних.
...
- Недоліки реалізацій, які можуть мати певні обмеження та висувати певні вимоги, наприклад, вимога власної схеми бази даних та обмеження коштом створення бази даних. Також може бути відсутня можливість написати в явному вигляді SQL-запит.
...
Якщо говорити про головний мінус ORM, зниження продуктивності, то причина цього полягає в тому, що більшість ORM націлені на обробку значної кількості різних сценаріїв використання даних, ніж у випадку окремої програми. У випадку невеликих проектів, які не стикаються з високим навантаженням, застосування ORM очевидне, особливо якщо врахувати такий важливий критерій розробки, як час. «Те, що легко пишеться з використанням ORM за тиждень, можна реалізовувати жоден місяць власними зусиллями»

2. В чому полягає різниця між JPA та Hibernate?

JPA це специфікація, а Hibernate – це ORM фреймворк/бібліотека, яка сумісна з JPA. Як і в класичному ООП, клас, який реалізує інтерфейс, може мати додаткові методи/властивості, так і Hiberante має більше фічі ніж JPA. За деякими даними, сама специфікація JPA була створена на базі ідей, втілених у Hibernate, проте офіційною реалізацією є EclipseLink. 

Тобто JPA тільки описує правила і API, а Hibernate реалізує ці описи, втім Hibernate (як і в багатьох інших реалізацій JPA) є додаткові можливості, не описані в JPA (і не переносяться на інші реалізації JPA).

3. Поясніть призначення кожного методу з інтерфейсу CrudRepository.

**save()**:

Зберігає вказану сутність. Використовуйте повернутий екземпляр для подальших операцій, оскільки операція збереження могла повністю змінити екземпляр сутності.

Синтаксис:
```
 <S extends T> S save(S entity)
```
- Параметри: entity – не має бути нульовим.

- Повертає: збережену сутність; ніколи не буде нульовим.

- Видає: IllegalArgumentException – у випадку, якщо дана сутність має значення null.

**findById()**: 

Отримує сутність за її ідентифікатором.
...
- Параметри: id – не має бути нульовим.

- Повертає: сутність із заданим ідентифікатором або Optional#empty(), якщо нічого не знайдено.

- Створюється виняток: IllegalArgumentException генерується, якщо 'id' дорівнює нулю.

**findAll()**: 

Повертає всі екземпляри типу.
...
- Повертає: усі сутності

**count()**: 

Повертає кількість доступних об’єктів.
...
- Видає: IllegalArgumentException - якщо дана сутність нульова.
OptimisticLockingFailureException - коли сутність використовує оптимістичне блокування та має атрибут версії зі значенням, відмінним від того, що міститься в сховищі збереження. Також викидається, якщо передбачається, що сутність присутня, але не існує в базі даних.

**deleteById()**

Видаляє сутність із заданим ідентифікатором.
...
- Видає: IllegalArgumentException- у випадку, якщо заданий ідентифікатор нульовий

**deleteAll()**

Видаляє всі сутності, якими керує репозиторій.
...
Клас сутності повинен відповідати цим вимогам:

- Клас повинен бути анотований javax.persistence.Entity.

- Клас повинен мати відкритий або захищений конструктор без аргументів. Клас може мати інші конструктори.

- Клас не можна оголошувати final. Жодні методи чи постійні змінні екземпляра не повинні оголошуватися як final.

- Якщо екземпляр сутності передається за значенням як відокремлений об’єкт, наприклад через віддалений бізнес-інтерфейс сеансового компонента, клас повинен реалізувати інтерфейс Serializable.
...
- Постійні змінні екземпляра мають бути оголошені приватними, захищеними або приватними для пакетів, і до них можна отримати прямий доступ лише за допомогою методів класу сутності. Клієнти повинні отримати доступ до стану сутності за допомогою методів доступу або бізнес-методів.

5. Які є типи відношень між сутностями у JPA?

- **ManyToOne**:

Відношення «багато до одного» між сутностями: де сутність пов’язана одна з одною (стовпцем або групою стовпців), яка містить унікальні значення. Стовпець або набір стовпців. Ці зв'язки застосовуються до реляційних баз даних із використанням зовнішнього ключа між таблицями.

- **OneToMany**: 

У цьому зв’язку кожен рядок однієї сутності посилається на багато дочірніх записів в іншій сутності. Важливо те, що записи дітей не можуть мати кількох батьків. У зв’язку «один-до-багатьох» між таблицею A та таблицею B кожен рядок таблиці A пов’язаний з 0, 1 або багатьма рядками таблиці B.

- **OneToOne**: 

У відносинах «один-до-одного» один елемент може належати лише одному іншому елементу. Це означає, що кожен рядок однієї сутності посилається на один і лише один рядок іншої сутності.

- **ManyToMany**: 

Зв’язок «багато-до-багатьох» полягає в тому, що один або кілька рядків з однієї сутності пов’язані з кількома рядками в іншій сутності.

6. Для чого потрібні DTO? Чому замість них не завжди можна використовувати @Entity об’єкти?

DTO — це не що інше, як клас-контейнер, який надає властивості, але не надає методи. DTO корисний, коли вам потрібно згрупувати значення в спеціальні структури для передачі даних.

З точки зору простого дизайну, DTO – це рішення, яке дійсно близьке до досконалості. DTO допомагають ще більше відокремити презентацію від рівня обслуговування та моделі домену. Коли використовуються DTO, рівень презентації та рівень обслуговування спільно використовують контракти даних, а не класи. Контракт даних — це, по суті, нейтральне представлення даних, якими обмінюються взаємодіючі компоненти. Контракт даних описує дані, які отримує компонент, але це не системний клас, як сутність. Зрештою, контракт даних — це клас, але він більше схожий на допоміжний клас, спеціально створений для конкретного методу обслуговування.
